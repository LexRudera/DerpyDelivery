...
namespace me
{
    ...
    class InputManager
    {
        public:
            ...
            // Keyboard
            bool IsKeyPressed(const sf::Keyboard::Key& key) { return KeysPressed[key]; }
            bool IsKeyDown(const sf::Keyboard::Key& key) { return KeysDown[key]; }
            bool IsKeyUp(const sf::Keyboard::Key& key) { return KeysUp[key]; }
            // Mouse
            bool IsButtonPressed(const sf::Mouse::Button& key) { return ButtonsPressed[key]; }
            bool IsButtonDown(const sf::Mouse::Button& key) { return ButtonsDown[key]; }
            bool IsButtonUp(const sf::Mouse::Button& key) { return ButtonsUp[key]; }
            sf::Vector2i GetMousePos() { return sf::Mouse::getPosition(*Game::Get()->GetWindow()); }
            sf::Vector2i GetAbsoluteMousePos() { return sf::Mouse::getPosition(); }
        protected:
        private:
            // Keyboard
